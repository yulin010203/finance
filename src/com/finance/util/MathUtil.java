...
public class MathUtil {
	...
	public static double round(double input, int scale) {
		...
		double pow = scale == 0 ? 1.0 : Math.pow(10, scale);
		double t = input * pow;
		if (input > 0) {
			return Math.round(t + 0.001) / pow;
		} else {
			return Math.round(t - 0.001) / pow;
		}
	}
	...
	public static String roundFromPoint(double input, int dig) {
		...
		if (input > 1) {
			int pointLength = inputString.length() - (pointIndex + 1);
			return pointLength > dig ? inputString.substring(0, pointIndex + 1 + dig) : inputString;
		}
		int fromIndex = 0;
	...
	public static void main(String[] argv) {
		...
		System.out.println(roundFromPoint(15000.9999, 2));
		System.out.println(roundFromPoint(0.000002555, 2));
		System.out.println(roundFromPoint(1.55555, 2));
	}
